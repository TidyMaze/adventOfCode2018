...
val interestingOnes = input.split("\n").toList.map(_.groupBy(identity).filter(m => m._2.size == 2 || m._2.size == 3))

val thoseDoubled = interestingOnes.count(_.exists(_._2.size == 2))

val thoseTripled = interestingOnes.count(_.exists(_._2.size == 3))

val res = thoseDoubled * thoseTripled
