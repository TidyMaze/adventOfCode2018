...
val regexWakesUp = """^\[(\d+)-(\d+)-(\d+) (\d+):(\d+)\] wakes up$""".r
val regexBeginShift = """^\[(\d+)-(\d+)-(\d+) (\d+):(\d+)\] Guard #(\d+) begins shift$""".r
val regexFallsAsleep = """^\[(\d+)-(\d+)-(\d+) (\d+):(\d+)\] falls asleep$""".r

sealed trait Action extends Product with Serializable {
  def date: LocalDateTime
}
case class WakesUp(time: LocalDateTime) extends Action {
  override def date = time
}
case class BeginShift(time: LocalDateTime, guard: Int) extends Action {
  override def date = time
}
case class FallsAsleep(time: LocalDateTime) extends Action {
  override def date = time
}
...
val actions =
  """[1518-09-14 00:54] wakes up
    ...
    |[1518-05-02 00:27] falls asleep""".stripMargin.split("\n").map {
  case regexBeginShift(year, month, day, hour, minutes, guard) =>
    BeginShift(LocalDateTime.of(year.toInt, month.toInt, day.toInt, hour.toInt, minutes.toInt), guard.toInt)
  case regexWakesUp(year, month, day, hour, minutes) =>
    WakesUp(LocalDateTime.of(year.toInt, month.toInt, day.toInt, hour.toInt, minutes.toInt))
  case regexFallsAsleep(year, month, day, hour, minutes) =>
    FallsAsleep(LocalDateTime.of(year.toInt, month.toInt, day.toInt, hour.toInt, minutes.toInt))
}.toSeq

implicit val localDateOrdering: Ordering[LocalDateTime] = Ordering.by(_.toEpochSecond(ZoneOffset.UTC))

val sorted = actions.sortBy(_.date)

sorted.mkString("\n")

val endState: (Option[Int], Option[Int], Map[Int, Int], Map[(Int, Int), Int]) = sorted.foldLeft((Option.empty[Int], Option.empty[Int], Map.empty[Int, Int], Map.empty[(Int, Int), Int])){
  case ((_, sleepingSince, totalSleep, minutesSlept), BeginShift(_, id)) =>           (Some(id), sleepingSince, totalSleep, minutesSlept)
  case ((Some(id), sleepingSince, totalSleep, minutesSlept), FallsAsleep(time)) =>    (Some(id), Some(time.toLocalTime.getMinute), totalSleep, minutesSlept)
  case ((Some(id), Some(sleepingSince), totalSlept, minutesSlept), WakesUp(time)) =>
    (
      Some(id),
      None,
      totalSlept + (id -> (totalSlept.getOrElse(id, 0) + (time.toLocalTime.getMinute - sleepingSince))),
      minutesSlept ++ (sleepingSince until time.getMinute).map { minute =>
        (id, minute) -> (minutesSlept.getOrElse((id, minute), 0) + 1)
      }
    )
}

val guardId = endState._3.maxBy(_._2)._1
val minuteTarget = endState._4.filter(_._1._1 == guardId).maxBy(_._2)._1._2

val res = guardId * minuteTarget