...
def shared(w: String, w2: String): String =
  (w zip w2).filter(v => v._1 == v._2).map(_._1).mkString
...
(for {
  l <- lines
  l2 <- lines
  allShared = shared(l, l2) if allShared.length == l.length - 1
} yield allShared).toSet
